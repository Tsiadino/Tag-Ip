defmodule EventDefinitionWeb.EventNewLive do
  ...
  def handle_event("save", %{"event_definition" => params}, socket) do
    case Ash.create(EventDefinition.Events.EventDefinition, params,
      ...
      %{changeset: %{errors: field_errors}} when is_list(field_errors) ->
        Enum.flat_map(field_errors, fn field_error ->
          case field_error do
            ...
            %{fields: fields, message: message} when is_list(fields) ->
              Enum.map(fields, &{to_string(&1), message})

            %{field: field} ->
              [{to_string(field), Exception.message(field_error)}]
  ...
  defp field_name(%{fields: [field | _]}), do: to_string(field)
